...
import {
  ConwayPParamsPoolVotingThresholds,
  isConwayPParamsPoolVotingThresholds,
  conwayPoolVotingThresholdsToCborObj,
  tryGetConwayPParamsPoolVotingThresholdsFromCborObj,
} from "./ConwayPParamsPoolVotingThresholds";
import {
  ConwayPParamsDrepVotingThresholds,
  isConwayPParamsDrepVotingThresholds,
  conwayDrepVotingThresholdsToCborObj,
  tryGetConwayPParamsDrepVotingThresholdsFromCborObj,
} from "./ConwayPParamsDrepVotingThresholds";
import {
  Rational,
  isRational,
  isRationalOrUndefined,
  tryCborFromRational,
} from "../../common";
...
export function isConwayProtocolParameters(
  something: any,
): something is ConwayProtocolParameters {
  const expectedKeys = [
    "txFeePerByte",
    "txFeeFixed",
    "maxBlockBodySize",
  ...
  if (
    ...
    ).every((uintKey) => canBeUInteger(pp[uintKey]))
  )
    return false;
...
export function isPartialConwayProtocolParameters(
  something: object,
): something is Partial<ConwayProtocolParameters> {
  ...
  if (
    !(
      ...
      (uintKey) => pp[uintKey] === undefined || canBeUInteger(pp[uintKey]),
    )
  )
    return false;
  ...
  if (
    !(
      ...
      (pp.poolVotingThresholds === undefined ||
        isConwayPParamsPoolVotingThresholds(pp.poolVotingThresholds)) &&
      (pp.drepVotingThresholds === undefined ||
        isConwayPParamsDrepVotingThresholds(pp.drepVotingThresholds))
    )
  )
    return false;
...
function mapUIntEntryOrUndefined(
  tag: number,
  a: CanBeUInteger | undefined,
): { k: CborUInt; v: CborUInt } | undefined {
  return a === undefined
    ? undefined
    : {
        k: new CborUInt(tag),
...
export function partialConwayProtocolParametersToCborObj(
  pps: Partial<ConwayProtocolParameters>,
): CborMap {
  const {
    executionUnitPrices,
    maxTxExecutionUnits,
    ...
    [
      ...
      kv(
        ...
        costModels === undefined ||
          (!costModelsKeys.includes("PlutusV1") &&
            !costModelsKeys.includes("PlutusV2") &&
            !costModelsKeys.includes("PlutusV3"))
          ? undefined
          : costModelsToCborObj(costModels),
      ),
      executionUnitPrices === undefined
        ? undefined
        : kv(
            19,
            Array.isArray(executionUnitPrices)
              ...
              : new CborArray([
                  CborPositiveRational.fromNumber(
      ...
      kv(
        20,
        ExBudget.isJson(maxTxExecutionUnits)
          ? ExBudget.fromJson(maxTxExecutionUnits).toCborObj()
          : maxTxExecutionUnits?.toCborObj(),
      ),
      kv(
        21,
        ExBudget.isJson(maxBlockExecutionUnits)
          ? ExBudget.fromJson(maxBlockExecutionUnits).toCborObj()
          : maxBlockExecutionUnits?.toCborObj(),
      ),
      ...
      pps.poolVotingThresholds
        ? kv(25, conwayPoolVotingThresholdsToCborObj(pps.poolVotingThresholds))
        : undefined,
      pps.drepVotingThresholds
        ? kv(26, conwayDrepVotingThresholdsToCborObj(pps.drepVotingThresholds))
        : undefined,
...
export function partialConwayProtocolParametersToData(
  pps: Partial<ConwayProtocolParameters>,
): Data {
...
function cborToDataLitteral(cbor: CborObj): Data {
  ...
  if (cbor instanceof CborMap) {
    return new DataMap(
      cbor.map.map(
        ({ k, v }) =>
          new DataPair(cborToDataLitteral(k), cborToDataLitteral(v)),
      ),
    );
  }
...
export function partialConwayProtocolParametersFromCborObj(
  cObj: CborObj,
): Partial<ConwayProtocolParameters> {
  if (!(cObj instanceof CborMap))
    throw new Error(
      `Invalid CBOR format for "Partial<ConwayProtocolParameters>"`,
    );

  let fields: (CborObj | undefined)[] = new Array(
    maxProtocolParamsEntries,
  ).fill(undefined);
  ...
  let executionUnitPrices:
    | [CborPositiveRational, CborPositiveRational]
    | undefined = undefined;
  if (_execCosts instanceof CborArray) {
    const mem_price = CborPositiveRational.fromCborObjOrUndef(
      _execCosts.array[0],
    );
    const cpu_price = CborPositiveRational.fromCborObjOrUndef(
      _execCosts.array[1],
    );
    executionUnitPrices =
      mem_price !== undefined && cpu_price !== undefined
        ? [mem_price, cpu_price]
        : undefined;
  }
  ...
  return {
    ...
    poolPledgeInfluence:
      CborPositiveRational.fromCborObjOrUndef(_pledgeInfluence),
    ...
    executionUnitPrices,
    maxTxExecutionUnits:
      _maxTxExecUnits === undefined
        ? undefined
        : ExBudget.fromCborObj(_maxTxExecUnits),
    maxBlockExecutionUnits:
      _maxBlockExecUnits === undefined
        ? undefined
        : ExBudget.fromCborObj(_maxBlockExecUnits),
    ...
    poolVotingThresholds: tryGetConwayPParamsPoolVotingThresholdsFromCborObj(
      _poolVotingThresholds,
    ),
    drepVotingThresholds: tryGetConwayPParamsDrepVotingThresholdsFromCborObj(
    ...
    minfeeRefScriptCostPerByte: tryCborFromRational(
      _minfeeRefScriptCostPerByte,
    ),
  };
}

export const defaultConwayProtocolParameters: ConwayProtocolParameters =
  freezeAll({
...
export function partialConwayProtocolParamsToJson(
  pp: Partial<ConwayProtocolParameters>,
) {
  return {
    ...pp,
    poolPledgeInfluence:
      typeof pp.poolPledgeInfluence === "number"
        ? pp.poolPledgeInfluence
        : pp.poolPledgeInfluence?.toNumber(),
    monetaryExpansion:
      typeof pp.monetaryExpansion === "number"
        ? pp.monetaryExpansion
        : pp.monetaryExpansion?.toNumber(),
    treasuryCut:
      typeof pp.treasuryCut === "number"
        ? pp.treasuryCut
        : pp.treasuryCut?.toNumber(),
    costModels:
      pp.costModels === undefined ? undefined : costModelsToJson(pp.costModels),
    ...
    maxTxExecutionUnits: ExBudget.isJson(pp.maxTxExecutionUnits)
      ? pp.maxTxExecutionUnits
      : pp.maxTxExecutionUnits?.toJson(),
    maxBlockExecutionUnits: ExBudget.isJson(pp.maxBlockExecutionUnits)
      ? pp.maxBlockExecutionUnits
      : pp.maxBlockExecutionUnits?.toJson(),
  };
}
