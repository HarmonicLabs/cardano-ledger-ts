...
export function isIMaryHeaderBody( thing: any ): thing is IMaryHeaderBody
{
    return isObject( thing ) && (
        thing instanceof MaryHeaderBody ||
        (
    ...
    implements IMaryHeaderBody, ToCbor, IPraosHeaderBody
{
    ...
    {
        // console.log("cHdrBody Mary", cHdrBody);
        if (!(
            cHdrBody instanceof CborArray 
            // && cHdrBody.array.length === 15
        ))throw new InvalidCborFormatError("invalid cbor for MaryHeaderBody; expected array of 15 elements");
        ...
        if (!(
            ...
            throw new Error("invalid types in MaryHeaderBody");
        }
