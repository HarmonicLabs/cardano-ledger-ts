...
export interface IDijkstraTxOut {
...
export function isIDijkstraTxOut( stuff: any ): stuff is IDijkstraTxOut
{
    return (
        isObject( stuff ) &&
...
export class DijkstraTxOut
    implements IDijkstraTxOut, ToCbor, Cloneable<DijkstraTxOut>, ToData, ToJson
{
    ...
    constructor(
        DijkstraTxOutput: IDijkstraTxOut,
        readonly cborRef: SubCborRef | undefined = undefined
    )
    {
        if(!(
            isObject( DijkstraTxOutput ) &&
            hasOwn( DijkstraTxOutput, "address" ) &&
            hasOwn( DijkstraTxOutput, "value" )
        )) throw new Error("DijkstraTxOutput is missing some necessary fields");
        ...
        )) throw new Error("invalid 'address' while constructing 'DijkstraTxOut'");

        if(!(
            value instanceof Value
        )) throw new Error("invalid 'value' while constructing 'DijkstraTxOut'");
        ...
        this.cborRef = cborRef ?? subCborRefOrUndef( DijkstraTxOutput );
    }

    clone(): DijkstraTxOut
    {
        return new DijkstraTxOut({
    ...
    toCborObj(): CborMap
    {
        ...
        if( !Value.isPositive( this.value ) )
        {
            ...
            throw new BasePlutsError("DijkstraTxOut values can only be positive; value was: " + JSON.stringify( this.value.toJson() ));
        }
        return new CborMap([
            {
                k: new CborUInt( 0 ),
    ...
    static fromCbor( cStr: CanBeCborString ): DijkstraTxOut
    {
        return DijkstraTxOut.fromCborObj( Cbor.parse( forceCborString( cStr ), { keepRef: true } ) );
    }
    static fromCborObj( cObj: CborObj ): DijkstraTxOut
    {
        if(!(
        ...
        )) throw new InvalidCborFormatError("DijkstraTxOut");
        ...
        if( _dat !== undefined )
        {
            ...
            else throw new InvalidCborFormatError("DijkstraTxOut");
