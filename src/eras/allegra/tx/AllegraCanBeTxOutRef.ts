...
import { IAllegraUTxO, isIAllegraUTxO } from "./AllegraUTxO";


export type CanBeTxOutRef = ITxOutRef | IAllegraUTxO | TxOutRefStr;

export function forceTxOutRefStr( canBe: CanBeTxOutRef ): TxOutRefStr
{
    return typeof canBe === "string" ? canBe : ITxOutRefToStr( isIAllegraUTxO( canBe ) ? canBe.utxoRef : canBe )
}
...
export function forceTxOutRef( canBe: CanBeTxOutRef ): TxOutRef
{
    ...
    else _interface = isIAllegraUTxO( canBe ) ? canBe.utxoRef : canBe;
