...
export function isProtocolParameters(something: any): something is ProtocolParameters {
	...
	if (
		!(
			// protocolVersion removed in conway
			(ppv === undefined || isIProtocolVersion(ppv))
		)
	)
		return false;
	...
	if (!(isCostModels(pp.costModels) && isPParamsPoolVotingThresholds(pp.poolVotingThresholds) && isPParamsDrepVotingThresholds(pp.drepVotingThresholds))) return false;
...
export function isPartialProtocolParameters(something: object): something is Partial<ProtocolParameters> {
	...
	if (!(ppv === undefined || isIProtocolVersion(ppv))) return false;
	...
	if (!((pp.costModels === undefined || isCostModels(pp.costModels)) && (pp.poolVotingThresholds === undefined || isPParamsPoolVotingThresholds(pp.poolVotingThresholds)) && (pp.drepVotingThresholds === undefined || isPParamsDrepVotingThresholds(pp.drepVotingThresholds)))) return false;
...
export function partialProtocolParametersToCborObj(pps: Partial<ProtocolParameters>): CborMap {
	const { protocolVersion, executionUnitPrices, maxTxExecutionUnits, maxBlockExecutionUnits, costModels } = pps;
	...
	return new CborMap(
		[
			...
			protocolVersion === undefined ? undefined : kv(14, new ProtocolVersion(protocolVersion).toCborObj()),
			mapUIntEntryOrUndefined(16, pps.minPoolCost),
			mapUIntEntryOrUndefined(17, pps.utxoCostPerByte),
			kv(18, costModels === undefined || (!costModelsKeys.includes('PlutusV1') && !costModelsKeys.includes('PlutusV2')) ? undefined : costModelsToCborObj(costModels)),
			executionUnitPrices === undefined ? undefined : kv(19, Array.isArray(executionUnitPrices) ? new CborArray(executionUnitPrices) : new CborArray([CborPositiveRational.fromNumber(executionUnitPrices.priceSteps), CborPositiveRational.fromNumber(executionUnitPrices.priceMemory)])),
...
export function partialProtocolParametersFromCborObj(cObj: CborObj): Partial<ProtocolParameters> {
	if (!(cObj instanceof CborMap)) throw new Error(`Invalid CBOR format for "Partial<ProtocolParameters>"`);
...
export const defaultProtocolParameters: ProtocolParameters = freezeAll({
	...
	protocolVersion: new ProtocolVersion({ major: 10, minor: 0 }),
	...
	maxTxExecutionUnits: new ExBudget({ mem: 16_500_000, cpu: 10_000_000_000 }),
	...
	minfeeRefScriptCostPerByte: CborPositiveRational.fromNumber(15.0),
